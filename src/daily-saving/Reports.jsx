...
import { Download, FileText, FileSpreadsheet, Calendar, Building2, TrendingUp, Users, IndianRupee, MapPin } from "lucide-react"
...
// Updated for Daily Savings / Area
const areaData = [
  { name: "Manpur Area Group", collection: 850000, members: 45, status: "Active" },
  { name: "Sikrai Local Hub", collection: 680000, members: 30, status: "Active" },
  { name: "Sikandra Central", collection: 1250000, members: 60, status: "Active" },
  { name: "Dausa City Pool", collection: 450000, members: 25, status: "Active" },
  { name: "Bandikui Region", collection: 800000, members: 40, status: "Completed" },
]
...
const reportTypesData = [
  ...
  { id: "area", name: "Area-wise Report", description: "Performance report by local area", icon: MapPin },
  { id: "member", name: "Member Payment Report", description: "Individual member payment history", icon: Users },
  { id: "agent", name: "Agent Performance Report", description: "Agent daily collection performance", icon: TrendingUp },
]
    ...
    <div className="space-y-6 animate-in fade-in duration-200 text-xs font-bold">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between border-b border-slate-200/60 dark:border-slate-800/60 pb-5">
        <div>
          ...
          <p className="text-slate-500 dark:text-slate-400 text-[12px] font-bold mt-0.5">
            Audit system spreadsheets and evaluate dynamic daily collection charts and summaries.
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 font-sans">
        <div className="bg-white dark:bg-[#0f172a] border border-slate-200 dark:border-slate-800 rounded-2xl p-4 flex items-center gap-3.5 shadow-2xs transition-colors duration-300">
          ...
          <div>
            <p className="text-slate-400 text-[11px] font-bold uppercase tracking-wider">Net Area Collection</p>
        ...
        <div className="bg-white dark:bg-[#0f172a] border border-slate-200 dark:border-slate-800 rounded-2xl p-4 flex items-center gap-3.5 shadow-2xs transition-colors duration-300">
          <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-blue-50 dark:bg-blue-950/40 text-blue-600 dark:text-blue-400 border border-blue-100/30 shadow-inner"><MapPin className="h-5 w-5" /></div>
          <div>
            <p className="text-slate-400 text-[11px] font-bold uppercase tracking-wider">Active Areas</p>
            <p className="text-xl font-black text-slate-900 dark:text-white mt-0.5 font-mono tracking-tight">12 Hubs</p>
          </div>
        </div>
      ...
      <div className="bg-white dark:bg-[#0f172a] border border-slate-200 dark:border-slate-800 rounded-2xl p-5 shadow-2xs transition-colors">
        <div className="flex flex-wrap items-center gap-5 font-semibold">
          ...
          <div className="space-y-1.5 text-left">
            <label className="text-slate-500 dark:text-slate-400 uppercase text-[10px] font-bold pl-0.5">Target Area Group</label>
            <select 
              value={selectedArea} 
              onChange={(e) => setSelectedArea(e.target.value)}
              className="bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl px-4 py-2.5 text-xs font-bold text-slate-700 dark:text-slate-300 focus:outline-none focus:border-blue-500 cursor-pointer w-48"
            >
              <option value="all">All Areas Matrices</option>
              ...
              <option value="bandikui">Bandikui Region</option>
            </select>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-white dark:bg-[#0f172a] border border-slate-200 dark:border-slate-800 rounded-2xl p-6 flex flex-col shadow-xs transition-colors">
          <div className="mb-4 text-left">
            <h3 className="text-[13px] font-extrabold uppercase tracking-wider text-slate-400 dark:text-slate-500">Collection Trend Curves</h3>
      ...
      {/* Area-wise Grid Table Documentation Sheet */}
      <div className="bg-white dark:bg-[#0f172a] border border-slate-200 dark:border-slate-800 rounded-2xl shadow-xs overflow-hidden transition-all duration-300">
        <div className="p-5 border-b border-slate-100 dark:border-slate-800 flex flex-row items-center justify-between bg-white dark:bg-[#0f172a]">
          <div className="text-left">
            <h3 className="text-[13px] font-extrabold uppercase tracking-wider text-slate-800 dark:text-slate-200">Area-wise Analysis Sheet</h3>
            <p className="text-slate-500 dark:text-slate-400 text-[12px] font-bold mt-0.5">Detailed documentation overview map across local operations.</p>
          </div>
          ...
          <table className="w-full text-left border-collapse font-sans">
            <thead>
              <tr className="border-b border-slate-100 dark:border-slate-800 text-[11px] uppercase tracking-wider text-slate-400 dark:text-slate-500 font-bold bg-slate-50/50 dark:bg-slate-950/20 select-none">
                <th className="p-4 pl-6">Area Identifier Module</th>
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800 text-[12px] font-bold text-slate-700 dark:text-slate-300">
              {areaData.map((area) => (
                <tr key={area.name} className="hover:bg-slate-50/60 dark:hover:bg-[#1e293b]/20 transition-colors">
                  <td className="p-4 pl-6 font-extrabold text-slate-900 dark:text-white text-[13px]">{area.name}</td>
                  <td className="p-4 text-right font-mono font-bold text-slate-800 dark:text-slate-200">{area.members} Operators</td>
                  <td className="p-4 text-right font-mono text-blue-600 dark:text-blue-400 text-[13px] font-black">₹{area.collection.toLocaleString()}</td>
                  <td className="p-4 pl-12">
                    <span className={`text-[9px] font-extrabold border px-2.5 py-0.5 rounded-md uppercase tracking-wider ${area.status === "Active" ? "bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border-emerald-500/20" : "bg-blue-500/10 text-blue-600 dark:text-blue-400 border-blue-500/20"}`}>
                      {area.status}
                    </span>
                  </td>
